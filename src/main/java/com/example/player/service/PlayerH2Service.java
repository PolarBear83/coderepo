...
public class PlayerH2Service implements PlayerRepository {
    ...
    public Player addPlayer(Player player){
        ad.update("insert into team(playerName, jerseryNumber, role values (?, ?, ?)", player.getPlayerName(), player.getJerseyNumber(), player.getRole());
    return db.queryForObject("select * from team where playerNamme = ? and jerseyNumber = ?", new PlayerRowMapper, player.getPlayerName(), player.getJerseyNumber());
    }
    ...
    @Override 
    public Player updatePlayer(int playerId, int Player player){
